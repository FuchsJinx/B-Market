...
public class DeliveryMethodDialog extends DialogFragment {
    ...
    public interface OnDeliveryMethodSelectedListener {
        ...
        void onPickupSelected(Cafe selectedCafe);
    }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        CafeAdapter adapter = new CafeAdapter(userLocation);
        ...
        btnSelectAddress.setBackgroundTintList(ContextCompat.getColorStateList(getContext(), R.color.soft_coffee_brown));
                ...
                .setPositiveButton("Подтвердить", (dialog, id) -> {
                    ...
                    if (selectedId == R.id.rb_pickup) {
                        if (adapter.getSelectedCafe() != null) {
                            ...
                            Toast.makeText(getContext(), "Выберите кафе для самовывоза", Toast.LENGTH_SHORT).show();
                        }
                    } else {
        ...
        dialog.setOnShowListener(d -> {
            ...
            if (positiveButton != null) {
                positiveButton.setTextColor(ContextCompat.getColor(getContext(), R.color.soft_coffee_brown));
            }
            if (negativeButton != null) {
                negativeButton.setTextColor(ContextCompat.getColor(getContext(), R.color.soft_coffee_brown));
            }
        });
    ...
    private void fetchCafes(CafeAdapter adapter) {
        FirebaseFirestore.getInstance().collection("cafes")
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    cafes = queryDocumentSnapshots.toObjects(Cafe.class);
                    sortCafesByDistance();
    ...
    private void sortCafesByDistance() {
        if (userLocation == null || cafes == null || cafes.isEmpty()) return;
        ...
        List<Pair<Cafe, Float>> cafeDistances = new ArrayList<>();
        for (Cafe cafe : cafes) {
            if (cafe.getLatitude() == 0 && cafe.getLongitude() == 0) continue;
            Location cafeLoc = new Location("cafe");
            cafeLoc.setLatitude(cafe.getLatitude());
            cafeLoc.setLongitude(cafe.getLongitude());
            cafeDistances.add(new Pair<>(cafe, userLoc.distanceTo(cafeLoc)));
        }

        Collections.sort(cafeDistances, (p1, p2) -> Float.compare(p1.second, p2.second));

        cafes.clear();
        for (Pair<Cafe, Float> pair : cafeDistances) {
            cafes.add(pair.first);
        }
    }
